...
export async function compareExportToDirectory(
  ...
  try {
    ...
    const compareObj2: CompareObj = { added, changed, deleted };
    saveJsonToFile(
      compareObj2,
      getFilePath("b1" + "fileDiff.config.json", true)
    );
...
async function emptyDirectory(dirPath: string): Promise<void> {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      new FrodoError(`Directory does not exist: ${absoluteDirPath}`);
      return;
    } else {
